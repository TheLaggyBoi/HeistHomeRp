...
function getgroup(source)
	...
	local result = MySQL.Sync.fetchAll('SELECT firstname, lastname, group FROM users WHERE identifier = @identifier', {
	...
	if result[1] and result[1].firstname and result[1].lastname then
		...
		return ('%s %s'):format(result[1].firstname, result[1].group)
	end
end