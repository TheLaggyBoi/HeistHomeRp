...
$(document).ready(function () {
  ...
  window.addEventListener('message', function (event) {
    ...
    if (item.response == "openGui") {
      ...
      if (itemList[item.id]) {
        $.post("http://np-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, true, itemList[item.id].nonStack, item.data, itemList[item.id].weight]));
      } else {
        $.post("http://np-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, false, false, item.data]));
      }
    }
  })
});
...
function DisplayInventory(sqlInventory, itemCount, invName, main) {
  ...
  if (failure) {
    BuildDrop(fixSlots)
    closeInv()
    console.log('FAULTY FUCKING SLOT YOU CUNT')
  }
