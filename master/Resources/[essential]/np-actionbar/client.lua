...
function attemptToDegWeapon()
	--Used to further degrade weapons after use, review later.
...
--[[Citizen.CreateThread( function() 
	while true do 
		...
		if hash ~= `WEAPON_UNARMED` then
			if weapon == "966099553" then
				--print("Faking joint m8")
			else
			if exports["np-inventory"]:hasEnoughOfItem(weapon,1,false) then
				--print("Working?")
			elseif not exports["np-inventory"]:hasEnoughOfItem(weapon,1,false) then
				armed = false
				TriggerEvent("hud-display-item",currentEquipped,"Removed")
				holster1h()
			end
		end
		else
...
Citizen.CreateThread( function()
	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			local weapon = "".. hash ..""
			currentEquipped = GetCurrentPedWeapon(PlayerPedId(), true)

			if throwableWeapons[currentEquipped] then

			else

			if ammoType == false then
...
AddEventHandler('equipWeaponID', function(hash)
	GiveAmmoNow()
