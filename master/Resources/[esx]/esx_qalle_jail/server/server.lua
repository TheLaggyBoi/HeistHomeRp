...
RegisterCommand("jail", function(src, args, raw)
	...
	if xPlayer["job"]["name"] == "police" then
		...
		local jailReason = args[3], args[4], args[5], args[6]
			...
			if jailTime ~= nil then
				...
				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						local fal = { "COURT NEWS : " ..Firstname .. " " .. Lastname .. " Is now in jail for ".. jailTime .." Months for the reason: " .. jailReason }
						local messageFull = {
							template = '<div style="padding: 8px; margin: 8px; background-color: rgba(255, 0, 0, 0.9); border-radius: 25px;"><i class="far fa-building"style="font-size:15px"></i>{1}</font></i></b></div>',
							args = {jobName, fal, msg}
						}
						TriggerClientEvent('chat:addMessage', -1, messageFull)
						--TriggerClientEvent('chat:addMessage', -1, { args = { "TIHAR NEWS",  Firstname .. " " .. Lastname .. " Is now in jail for ".. jailTime .." Months for the reason: " .. args[3] }, color = { 255, 0, 0 } })
					end)
				end
			else
