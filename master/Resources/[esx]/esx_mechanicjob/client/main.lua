				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
							...
							--TriggerEvent("ls:newVehicle", vehicle, platenew, nil)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 90.0, function(vehicle)
							...
							TriggerServerEvent('garage:addKeys', plate)
							--TriggerServerEvent("t1ger_keys:stolenCarKeys", plate)
							exports["mythic_notify"]:DoHudText('inform', 'Keys have been given to you' )
						end)
					else
...
--[[ function OpenMechanicHarvestMenu()
	if Config.EnablePlayerManagement and ESX.PlayerData.job and ESX.PlayerData.job.grade_name ~= 'novice' or ESX.PlayerData.job.grade_name ~= 'experimente' or ESX.PlayerData.job.grade_name ~= 'chief' or ESX.PlayerData.job.grade_name ~= 'boss' then
		local elements = {
		...
		exports['mythic_notify']:DoHudText('error', 'You are not Experienced Enough');
	end
end ]]
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elseif data.current.value == 'hijack_vehicle' then
			...
			if IsPedSittingInAnyVehicle(playerPed) then
				exports['mythic_notify']:DoHudText('error', 'You are inside the Vehicle');
				return
			end
		...
		elseif data.current.value == 'fix_vehicle' then
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					exports['mythic_notify']:DoHudText('success', 'Vehicle Repaired');
					isBusy = false
				end)
			else
				exports['mythic_notify']:DoHudText('error', 'No vehicle Nearby');
		...
		elseif data.current.value == 'del_vehicle' then
			...
			if IsPedSittingInAnyVehicle(playerPed) then
				...
				if GetPedInVehicleSeat(vehicle, -1) == playerPed then
					...
					exports['mythic_notify']:DoHudText('error', 'You must be in driver seat');
				end
			else
				local vehicle = ESX.Game.GetVehicleInDirection()
				...
				if DoesEntityExist(vehicle) then
					...
					exports['mythic_notify']:DoHudText('error', 'You must be near a Vehicle to impound it');
				end
			end
		elseif data.current.value == 'dep_vehicle' then
			...
			if isVehicleTow then
				...
				if CurrentlyTowedVehicle == nil then
					if targetVehicle ~= 0 then
						if not IsPedInAnyVehicle(playerPed, true) then
							if vehicle ~= targetVehicle then
								...
								exports['mythic_notify']:DoHudText('error', 'Vehicle Attached');

								if NPCOnJob then
									if NPCTargetTowable == targetVehicle then
										exports['mythic_notify']:DoHudText('inform', 'Please drop off the Vehicle at garage');
										Config.Zones.VehicleDelivery.Type = 1
								...
								exports['mythic_notify']:DoHudText('error', 'You cannot attach own towtruck');
							end
						end
					else
						exports['mythic_notify']:DoHudText('error', 'No vehicle to be Attached');
					end
				else
					...
					exports['mythic_notify']:DoHudText('success', 'Vehicle successfully detached');
				end
			else
				exports['mythic_notify']:DoHudText('error', 'You need a Flatbed to do that');
			end
	...
	while true do
		...
		if NPCTargetTowableZone and NPCHasSpawnedTowable and not NPCHasBeenNextToTowable then
			...
			if GetDistanceBetweenCoords(coords, zone.Pos.x, zone.Pos.y, zone.Pos.z, true) < Config.NPCNextToDistance then
				exports['mythic_notify']:DoHudText('inform', 'Please tow the Vehicle');
