...
local playerCoords = {}
local isDead, isHandcuffed, isShackles = false, false, false
local Keys = {
...
AddEventHandler('useparachute',function()
	if not HasPedGotWeapon(GetPlayerPed(-1), GetHashKey("GADGET_PARACHUTE"), false) then
		...
		TriggerEvent('esx:showNotification', "You Equipped Parachute")
	end
end)
...
AddEventHandler('esx_extraitems:bulletproof', function()
	...
	exports['progressBars']:startUI(5000, "Equipping Armour")
	TaskPlayAnim(playerPed, "missmic4", "michael_tux_fidget", 8.0, 2.0, 5000, 120, 1, 0, 0, 0)
	Citizen.Wait(5000)
...
-- End of Binoculars
------handcuff--------
RegisterNetEvent('extraitem:onhandcuffs')
AddEventHandler('extraitem:onhandcuffs', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
		if closestPlayer ~= -1 and closestDistance <= 3.0 then
			...
			TriggerServerEvent('extraitem:requestarrest', target_id, playerheading, playerCoords, playerlocation)
		else
			exports['mythic_notify']:DoHudText('error', "You wasted a handcuff, no one was around")
		end
end)

RegisterNetEvent('extraitem:getarrested')
AddEventHandler('extraitem:getarrested', function(playerheading, playercoords, playerlocation)
	...
	IsHandcuffed = true
	IsShackles = false
	--TriggerEvent('extraitem:handcuff')
	LoadAnimDict('mp_arresting')
	TaskPlayAnim(GetPlayerPed(-1), 'mp_arresting', 'idle', 8.0, -8, -1, 49, 0.0, false, false, false)
end)


RegisterNetEvent('extraitem:doarrested')
AddEventHandler('extraitem:doarrested', function()
	Citizen.Wait(250)
	LoadAnimDict('mp_arrest_paired')
...
---------uncuffs------
RegisterNetEvent('extraitem:onuncuffs')
AddEventHandler('extraitem:onuncuffs', function()
	local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
	if closestPlayer ~= -1 and closestDistance <= 3.0 then
		local target, distance = ESX.Game.GetClosestPlayer()
		playerheading = GetEntityHeading(GetPlayerPed(-1))
		...
		playerCoords = GetEntityCoords(GetPlayerPed(-1))
		local target_id = GetPlayerServerId(closestPlayer)
		TriggerServerEvent('extraitem:requestrelease', target_id, playerheading, playerCoords, playerlocation)
		Citizen.Wait(1200)
		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3, 'uncuff', 0.5)
	else
		exports['mythic_notify']:DoHudText('error', "no one is around")
	end
end)

RegisterNetEvent('extraitem:douncuffing')
AddEventHandler('extraitem:douncuffing', function()
	Citizen.Wait(250)
	LoadAnimDict('mp_arresting')
...
RegisterNetEvent('extraitem:getuncuffed')
AddEventHandler('extraitem:getuncuffed', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	SetEntityCoords(GetPlayerPed(-1), x, y, z)
	SetEntityHeading(GetPlayerPed(-1), playerheading)
	Citizen.Wait(250)
...
RegisterNetEvent('extraitem:loose')
AddEventHandler('extraitem:loose', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	SetEntityCoords(GetPlayerPed(-1), x, y, z)
	SetEntityHeading(GetPlayerPed(-1), playerheading)
	Citizen.Wait(250)
