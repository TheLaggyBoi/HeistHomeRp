  ...
  function(plate, type, item, count, max, owned)
    ...
    if type == "item_standard" then
     ...
     -- if targetItem.weight == -1 or ((targetItem.count + count) <= 50) then
              ...
              if coffre[i].name == item then
                if (coffre[i].count >= count and count > 0) then
                  ...
                  if (coffre[i].count - count) == 0 then
                    ...
                    local data3 = json.encode(coffre)
                    TriggerEvent("esx:trunkout", p, xPlayer, data3)
                  else
        ...
        TriggerClientEvent('big_notify:client:Notify', _source, { type = 'true', text = _U("invalid_quantity") })
        TriggerClientEvent('mythic_notify:SendAlert', _source, { type = 'error', text = _U("invalid_amount") })
                end
              end
            end
            ...
            TriggerClientEvent("conde_inventory:refreshTrunkInventory", _source, data, blackMoney, cashMoney, items, weapons)
          end
        )
      else
      
        TriggerClientEvent('big_notify:client:Notify', _source, { type = 'false', text = _U("player_inv_no_space") })
        TriggerClientEvent('mythic_notify:SendAlert', _source, { type = 'error', text = _U("player_inv_no_space") })
      end
    end
    ...
    if type == "item_account" then
      ...
      TriggerClientEvent('big_notify:client:Notify', _source, { type = 'true', text = _U("invalid_amount") })
      TriggerClientEvent('mythic_notify:SendAlert', _source, { type = 'error', text = _U("invalid_amount") })
          end
        end
      )
  ...
  function(plate, type, item, count, max, owned, label)
    ...
    if type == "item_standard" then
      ...
      if (playerItemCount >= count and count > 0) then
        ...
        TriggerClientEvent('big_notify:client:Notify', _source, { type = 'true', text = _U("insufficient_space") })
        TriggerClientEvent('mythic_notify:SendAlert', _source, { type = 'error', text = _U("insufficient_space") })
            else
              ...
              local p = json.encode(plate)
              local data = json.encode(coffre)
              TriggerEvent("esx:trunk", p, xPlayer, data)
            end
          end
        )
      else
