...
OpenControlMenu = function()
    local menuElements = {
        {
            ["label"] = "Place a new furniture.",
            ["action"] = "furnish"
        },
        {
            ["label"] = "Manage already existing furniture.",
            ["action"] = "refurnish"
        }
    }
...
OpenFurnishingMenu = function()
    ...
    if not cachedData["furnishings"][currentMotel] then return ESX.ShowNotification("You don't have any furniture purchased.") end
    if not cachedData["furnishings"][currentMotel]["ownedFurnishing"] or #cachedData["furnishings"][currentMotel]["ownedFurnishing"] == 0 then return ESX.ShowNotification("You don't have any furniture purchased.") end
...
ButtonPressed = function(keyLabel)
    ...
    if DoesEntityExist(furHandle) then
        ...
        elseif keyLabel == "ENTER" then
            DrawBusySpinner("Saving furnishing...")
            ...
            ESX.TriggerServerCallback("james_motels:saveFurnishing", function(savedFurnishing)
                if savedFurnishing then
                    ...
                    ESX.ShowNotification("You placed the furniture.")
                else
                    ESX.ShowNotification("Please try to place it again.")
                end
...
OpenFurnishingComputer = function()
    ...
    if not ownedMotel then return ESX.ShowNotification("You need to own a motel-room to manage the computer.") end
...
ComputerTick = function(currentProp)
    local PurchaseProp = function()
        ...
        ESX.TriggerServerCallback("james_motels:purchaseFurnishing", function(purchased)
            DrawBusySpinner("Verifying purchase...")
            ...
            if purchased then
                riggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Hype! Custom Styling!', length = 2500, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
            else
                riggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Hype! Custom Styling!', length = 2500, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
    ...
    Citizen.CreateThread(function()
        while DoesEntityExist(currentProp["handle"]) do
            ...
            DrawScriptText((cachedData["furnishingProp"]["coords"] or Config.MegaMall["object"]["pos"]) - vector3(0.0, 0.0, 0.25), "[~g~G~s~] tusuyla satın al.")
