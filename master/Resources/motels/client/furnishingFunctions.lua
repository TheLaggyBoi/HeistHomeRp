...
OpenControlMenu = function()
    local menuElements = {
        {
            ["label"] = "Set up a new piece of furniture.",
            ["action"] = "furnish"
        },
        {
            ["label"] = "Manage existing furniture.",
            ["action"] = "refurnish"
        }
    }
...
OpenFurnishingMenu = function()
    ...
    if not cachedData["furnishings"][currentMotel] then return TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'You didn\'t buy furniture, go to Ikea to buy it' } ) end
    if not cachedData["furnishings"][currentMotel]["ownedFurnishing"] or #cachedData["furnishings"][currentMotel]["ownedFurnishing"] == 0 then return TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'You didn\'t buy furniture, go to Ikea to buy it' } ) end
...
OpenRefurnishMenu = function()
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "refurnishing_motel_room", {
        ["title"] = "Choose furniture to manage.",
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
        local action = menuData["current"]["action"]
...
ButtonPressed = function(keyLabel)
    ...
    if DoesEntityExist(furHandle) then
        ...
        elseif keyLabel == "ENTER" then
            DrawBusySpinner("Furniture is being placed...")
            ...
            ESX.TriggerServerCallback("james_motels:saveFurnishing", function(savedFurnishing)
                if savedFurnishing then
                    ...
                    TriggerEvent('mythic_notify:client:SendAlert', { type = 'success', text = 'You Arranged The Furniture' } )
                else
                    TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'Try deploying again.' } )
                end
...
-- RegisterCommand("light", function(sources)
--     local furProp = cachedData["furnishingProp"]
--     local furHandle = furProp and cachedData["furnishingProp"]["handle"] or nil
--         local currentMotel = DecorGetInt(PlayerPedId(), "currentInstance")
...
OpenFurnishingComputer = function()
    ...
    if not ownedMotel then return TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = 'You must have a motel room to manage the computer' } ) end 
...
ComputerTick = function(currentProp)
    local PurchaseProp = function()
        ...
        ESX.TriggerServerCallback("james_motels:purchaseFurnishing", function(purchased)
            DrawBusySpinner("Confirming purchase..")
            ...
            if purchased then
                TriggerEvent('mythic_notify:client:SendAlert', { type = 'success', text = 'Purchased' } )
            else
                TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = 'Got a Problem, your money may be insufficient' } )
            end
    ...
    Citizen.CreateThread(function()
        while DoesEntityExist(currentProp["handle"]) do
            ...
            DrawScriptText((cachedData["furnishingProp"]["coords"] or Config.MegaMall["object"]["pos"]) - vector3(0.0, 0.0, 0.25), "[~g~G~s~] Buy it with the key.")
