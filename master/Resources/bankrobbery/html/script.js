...
var MainDigits = 5;
var RotationTime = 25000; // time ms
...
$(document).ready(function() {
    ...
    window.addEventListener('click', function() {
        ...
        if (distance < HitDistance) {
            ...
            MainDigits--;
            $.post("http://bankrobbery/thermiteclick");
...
Thermite.Failed = function() {
    if (active) $.post("http://bankrobbery/thermitefailed");
    active = false;
    Thermite.Close();
}

Thermite.Success = function() {
    if (active) $.post("http://bankrobbery/thermitesuccess");
    active = false;
    Thermite.Close();
}
...
Thermite.Close = function() {
    ...
    $.post('http://bankrobbery/closethermite');
}