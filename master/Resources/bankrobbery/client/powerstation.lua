...
AddEventHandler('robbery:UseThermite', function()
    ...
    if closestStation ~= 0 then
        ...
        if dist < 1.5 then
            ...
            function(online)
            if online >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    exports['mythic_notify']:DoHudText('inform', "It seems that the fuses have blown.")
                end
            else
                exports['mythic_notify']:DoHudText('inform', "Not enough police (2 needed)")
            end
            end)
        end
...
RegisterNetEvent('lh-bankrobbery:client:SetStationStatus')
AddEventHandler('lh-bankrobbery:client:SetStationStatus', function(key, isHit)
