...
ESX.RegisterServerCallback('carealeredm:GetDealerCount', function(source, cb, option)
...
AddEventHandler('carealeredm:CheckFinanceStatus', function()
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier",{['@identifier'] = xPlayer.identifier}, function(results) 
		...
		if foundOwedVeh then
			...
			TriggerClientEvent("carealeredm:ShowNotifyESX",xPlayer.source,_U('veh_repossessed_warning',warnTime))
...
-- ACCOUNT MENU:
ESX.RegisterServerCallback('carealeredm:GetAccountMoney', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('carealeredm:AccountsWithdraw', function(source, cb, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local account = MySQL.Sync.fetchAll('SELECT * FROM addon_account_data WHERE account_name=@account_name', {['@account_name'] = 'society_cardealer'})	
...
ESX.RegisterServerCallback('carealeredm:AccountsDeposit', function(source, cb, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local approved = false
	if Config.UseCashMoney then
...
-- EMPLOYEE MENU:
ESX.RegisterServerCallback('carealeredm:getEmployees', function(source, cb, society)		
	-- Fetch employees:
...
-- Set Job:
ESX.RegisterServerCallback('carealeredm:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.grade_name == 'boss' then
		...
		if xTarget then
			...
			if type == 'hire' then
				TriggerClientEvent('carealeredm:ShowNotifyESX', xTarget.source, _U('you_have_been_hired', job))
			elseif type == 'promote' then
				TriggerClientEvent('carealeredm:ShowNotifyESX', xTarget.source, _U('you_have_been_promoted'))
			elseif type == 'fire' then
				TriggerClientEvent('carealeredm:ShowNotifyESX', xTarget.source, _U('you_have_been_fired', xTarget.getJob().label))
			end
			cb()
		else
...
ESX.RegisterServerCallback('carealeredm:getOnlinePlayers', function(source, cb)
...
ESX.RegisterServerCallback('carealeredm:getJob', function(source, cb, society)
...
ESX.RegisterServerCallback('carealeredm:GetFinancingMoney', function(source, cb, dPayment, model, stock)	
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('carealeredm:SellOwnedVehicle', function(source, cb, plate, price, model)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('carealeredm:ShopGetPlyMoney', function(source, cb, model, price)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('carealeredm:GetPlayerMoney', function(source, cb, model, price, stock)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('cardealeredm:ticketcheck',function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('carealeredm:GetOwnedVehByPlate',function(source, cb, plate)	
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier",{['@identifier'] = xPlayer.identifier}, function(vehData) 
		...
		if not vehFound then
			TriggerClientEvent('carealeredm:ShowNotifyESX', xPlayer.source, _U('not_own_that_plate'))
		end
...
ESX.RegisterServerCallback('carealeredm:RepayAmount', function(source, cb, plate, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('carealeredm:GiveRegistrationPaper', function(player, target, plate)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner=@identifier',{['@identifier'] = xPlayer.identifier}, function(vehData) 
		...
		if vehFound then
			...
			TriggerClientEvent('carealeredm:ShowNotifyESX', xPlayer.source, _U('veh_owner_change'))
			TriggerClientEvent('carealeredm:ShowNotifyESX', tPlayer.source, _U('veh_owner_change2'))
		else
			TriggerClientEvent('carealeredm:ShowNotifyESX', xPlayer.source, _U('plate_not_exists'))
		end
	end)
end)
...
ESX.RegisterServerCallback("carealeredm:FetchData", function(source, cb)
	vehicles = {}
	display = {}
	categories = {}
	-- Fetch Data:
...
ESX.RegisterServerCallback('carealeredm:GetAllOwnedVehicles', function(source, cb, option)
    local xPlayer = ESX.GetPlayerFromId(source)
    local plyVehicles = {}
...
ESX.RegisterServerCallback('carealeredm:UpdateDealerStock', function(source, cb, VehModel)
    local xPlayer = ESX.GetPlayerFromId(source)
    -- Get Stock_
...
ESX.RegisterServerCallback('carealeredm:PlateInUse', function (source, cb, plate)
