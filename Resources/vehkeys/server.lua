 ...
 local ident = xPlayer.getIdentifier()
  ...
  if math.random(1, 10) >= item.chance then
   if tonumber(item.id) == 0 then
    ...
    TriggerClientEvent('mythic_notify:client:SendAlert', source, 'You found $'..item.quantity)
   elseif item.isWeapon then
    xPlayer.addWeapon(item.id, 50)
    TriggerClientEvent('mythic_notify:client:SendAlert', source, 'Item Added!')
   elseif tonumber(item.id) == 1 then
    TriggerClientEvent('mythic_notify:client:SendAlert', source, 'You have found the keys to the vehicle!')
    vehicleKeys[plate] = {}
...
AddEventHandler('garage:giveKey', function(target, plate)
 ...
 vehicleKeys[plate] = {}
 table.insert(vehicleKeys[plate], {id = ident})
 TriggerClientEvent('mythic_notify:client:SendAlert', targetSource, 'You just recieved keys to a vehicle')
...
AddEventHandler('removelockpick', function()
 ...
 if math.random(1, 20) == 1 then
  ...
  TriggerClientEvent('mythic_notify:client:SendAlert', source, 'The lockpick bent out of shape.')
 end
end)
