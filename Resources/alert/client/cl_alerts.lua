...
AddEventHandler('esx-outlawalert:combatInProgress', function(targetCoords)
	if ESX.GetPlayerData().job.name == 'police' then	
		if Config.gunAlert then
			...
			BeginTextCommandSetBlipName("STRING")              -- set the blip's leesxd caption
			AddTextComponentString('10-15 Fight in Progress')              -- to 'supermarket'
...
RegisterNetEvent('esx-outlawalert:carJackInProgress')
AddEventHandler('esx-outlawalert:carJackInProgress', function(targetCoords)
	if ESX.GetPlayerData().job.name == 'police' then	
		local alpha = 250
		--local targetCoords = GetEntityCoords(PlayerPedId(), true)
		local car = AddBlipForCoord(targetCoords.x, targetCoords.y, targetCoords.z)

		SetBlipScale(car, 1.5)
		SetBlipSprite(car,  148)
		SetBlipColour(car,  1)
		SetBlipAlpha(car, alpha)
		SetBlipHighDetail(car, true)
		SetBlipAsShortRange(car, true)
		BeginTextCommandSetBlipName("STRING")              -- set the blip's leesxd caption
		AddTextComponentString('10-67 Carjack in Progress')              -- to 'supermarket'
		EndTextCommandSetBlipName(car)
		SetBlipAsShortRange(car, 1)
		PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
...
RegisterNetEvent('esx-outlawalert:drugInProgress')
AddEventHandler('esx-outlawalert:drugInProgress', function(targetCoords)
	if ESX.GetPlayerData().job.name == 'police' then	
		if Config.gunAlert then
			local alpha = 250
			local targetCoords = GetEntityCoords(PlayerPedId(), true)
			...
			SetBlipScale(knife, 1.3)
			SetBlipSprite(knife,  140)
			SetBlipColour(knife,  0)
			SetBlipAlpha(knife, alpha)
			SetBlipAsShortRange(knife, true)
			BeginTextCommandSetBlipName("STRING")              -- set the blip's leesxd caption
			AddTextComponentString('10-38 Drug Sale in Progress')              -- to 'supermarket'
			EndTextCommandSetBlipName(knife)
			SetBlipAsShortRange(knife,  1)
			PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
