...
local function Craft(source)
	SetTimeout(4000, function()
		...
		if PlayersCrafting[source] == true then
			...
			local xPlayer = ESX.GetPlayerFromId(source)
			local GazBottleQuantity = xPlayer.getInventoryItem('steel').count

			if GazBottleQuantity <= 1 then
				--TriggerClientEvent('esx:showNotification', source, _U('not_enough_gas_can'))
				TriggerClientEvent('esx:showNotification', _source, _U('not_enough_gas_can'))
			else
...
function lockpick(source)
	local xPlayer = ESX.GetPlayerFromId(source)
	local GazBottleQuantity = xPlayer.getInventoryItem('steel').count

	if GazBottleQuantity <= 2 then
		--TriggerClientEvent('esx:showNotification', source, _U('not_enough_gas_can'))
		TriggerClientEvent('esx:showNotification', _source, _U('not_enough_gas_can'))
	else
		xPlayer.removeInventoryItem('steel', 2)
...
function bodykit(source)
	local xPlayer = ESX.GetPlayerFromId(source)
	local steelQuantity = xPlayer.getInventoryItem('steel').count
	local plasticQuantity = xPlayer.getInventoryItem('plastic').count
	local glassQuantity = xPlayer.getInventoryItem('glass').count
	local scrapmetalQuantity = xPlayer.getInventoryItem('scrapmetal').count

	if steelQuantity <= 1 and plasticQuantity <= 1 and glassQuantity <= 1 and scrapmetalQuantity <= 1 then
		--TriggerClientEvent('esx:showNotification', source, _U('not_enough_body_tools'))
		TriggerClientEvent('esx:showNotification', _source, _U('not_enough_body_tools'))
	else
		xPlayer.removeInventoryItem('steel', 1)
		Citizen.Wait(1500)
		xPlayer.removeInventoryItem('glass', 1)
		Citizen.Wait(1500)
...
function fixkit(source)
	...
	if  steelQuantity <= 5 and plasticQuantity <= 5 and glassQuantity <= 5 and scrapmetalQuantity <= 5 then
		--TriggerClientEvent('esx:showNotification', source, 'inform', _U('not_enough_repair_tools'))
		TriggerClientEvent('esx:showNotification', _source, _U('not_enough_repair_tools'))
	else
...
local function Craft2(source)
	SetTimeout(4000, function()
		if PlayersCrafting[source] == true then
			...
			if  steelQuantity <= 4 and plasticQuantity <= 4 and glassQuantity <= 4 and scrapmetalQuantity <= 4 then
				--TriggerClientEvent('esx:showNotification', source, 'inform', _U('not_enough_repair_tools'))
				TriggerClientEvent('esx:showNotification', _source, _U('not_enough_repair_tools'))
			else
				xPlayer.removeInventoryItem('steel', 5)
...
local function Craft3(source)
	SetTimeout(4000, function()
		if PlayersCrafting[source] == true then
			...
			if steelQuantity <= 0 and plasticQuantity <= 0 and glassQuantity <= 0 and scrapmetalQuantity <= 0 then
				TriggerClientEvent('esx:showNotification', source, _U('not_enough_body_tools'))
				--TriggerClientEvent('esx:showNotification', _source, _U('assembling_repair_kit'))
			else
				xPlayer.removeInventoryItem('steel', 1)
