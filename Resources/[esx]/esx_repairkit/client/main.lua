...
AddEventHandler('esx_repairkit:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		local vehicle = nil
			exports['mythic_notify']:DoHudText('error', 'not near engine');
		else
		...
		Citizen.CreateThread(function()
			...
			if IsControlJustPressed(0, Keys["X"]) and isReparing == true then
				...
				exports['mythic_notify']:DoHudText('error', 'repair aborted');
				isReparing = not isReparing
				CurrentAction = nil
...
AddEventHandler('tyrekit:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		local vehicle = nil
			...
			exports['mythic_notify']:DoHudText('error', 'not near a tyre');
		else
		...
		then
			...
			Citizen.CreateThread(function()
				...
				if CurrentAction ~= nil then
					...
					exports['mythic_notify']:DoHudText('success', 'tyre repaired');
				if isReparing == true then
					isReparing = not isReparing
				end
		...
		Citizen.CreateThread(function()
			...
			if IsControlJustPressed(0, Keys["X"]) and isReparing == true then
				...
				exports['mythic_notify']:DoHudText('error', 'aborted tyre repair');
				isReparing = not isReparing
				CurrentAction = nil
