	...
	while true do
		...
		local coords = GetEntityCoords(GetPlayerPed(-1))
			...
			if(GetDistanceBetweenCoords(coords, -491.460, -1746.190, 18.70, true) < 10.0) or (GetDistanceBetweenCoords(coords, -491.870, -1756.750, 19.20, true) < 10.0) or (GetDistanceBetweenCoords(coords, 220.870, 6501.150, 31.20, true) < 10.0) then
					...
					ESX.Game.Utils.DrawText3D({ x = -491.740, y = -1746.890, z = 19.30 }, '~y~[E] ~w~Scrap collection', 0.6)		
					ESX.Game.Utils.DrawText3D({ x = -491.470, y = -1756.750, z = 19.00 }, '~y~[E] ~w~Scrap collection', 0.6)	
					ESX.Game.Utils.DrawText3D({ x = -498.250, y = -1756.160, z = 19.00 }, '~y~[E] ~w~Scrap collection', 0.6)				
...
function zbieranie()
TriggerServerEvent('smerfikcraft:scrapmetaliarzzbier')
wtrakcie = true
end
	...
	while true do
		...
		if(GetDistanceBetweenCoords(coords, -429.870, -1728.850, 19.90, true) < 20.0) or (GetDistanceBetweenCoords(coords, -1040.660, 5329.250, 43.370, true) < 5.0) then
			...
			if(GetDistanceBetweenCoords(coords, -423.260, -1720.750, 19.370, true) < 3.0) then
				ESX.ShowHelpNotification('[E] to Sell')
				if IsControlJustReleased(0, 38) then
					TriggerEvent('odlacz:propa')
					skup()
				end
			end
...
RegisterCommand("rprop", function()
	TriggerEvent('odlacz:propa')
end)
...
local blips = {
	...
	{title="Scrap collection", colour=5, id=468, x = -491.240, y = -1753.890, z = 19.00},
	{title="Base of scrap metalists", colour=5, id=488, x = -430.060, y = -1726.750, z = 29.90},
	{title="Purchase of scrap metal", colour=5, id=479, x = -422.020, y = -1720.250, z = 43.370}
...
ESX.ShowHelpNotification('do /rprop to throw object')
end)
