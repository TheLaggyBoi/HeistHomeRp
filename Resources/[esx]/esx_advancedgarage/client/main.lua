...
function ListOwnedCarsMenu()
	...
	--ESX.TriggerServerCallback('tiger_cardealer:GetOwnedVehByPlate',function(ownedCars)
		if #ownedCars == 0 then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
			...
			if data.current.value.stored then
				menu.close()
				ESX.TriggerServerCallback('esx_advancedgarage:getVehicledamage', function(damages)
...
function StoreVehicle(vehicle, vehicleProps, damages)
	...
	TriggerServerEvent('esx_advancedgarage:setVehicleState', vehicleProps.plate, true, damages)
