...
AddEventHandler('conde-inventoryhud:steal', function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'mp_arresting', 'idle', 3) or IsEntityDead(searchPlayerPed) or GetEntityHealth(searchPlayerPed) <= 0 or IsEntityPlayingAnim(searchPlayerPed, "mp_arresting", "idle", 3) or IsEntityPlayingAnim(searchPlayerPed, "mp_arrest_paired", "crook_p2_back_right", 3) then
            exports['mythic_progbar']:Progress({
                ...
                if not status then
                    ...
                    TriggerServerEvent('inventory:confiscatePlayerItem',GetPlayerServerId(closestPlayer))
        ...
        exports['mythic_notify']:DoHudText('error', 'No players around')
    end
end)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('inventory:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
