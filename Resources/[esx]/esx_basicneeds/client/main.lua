...
AddEventHandler('esx_basicneeds:resetStatus', function()
	...
	TriggerEvent('esx_status:set', 'stress', 10000)
end)
	...
	Citizen.CreateThread(function()
		while true do
				...
				if status.val == 0 then
					if prevHealth <= 150 then
						...
						if not IsPedRagdoll(playerPed) and IsPedOnFoot(playerPed) and not IsPedSwimming(playerPed) then
							exports['mythic_notify']:DoHudText('inform', 'You are Feeling Hungry')
			...
			TriggerEvent('esx_status:getStatus', 'thirst', function(status)
				if status.val == 0 then
					if prevHealth <= 150 then
						...
						if not IsPedRagdoll(playerPed) and IsPedOnFoot(playerPed) and not IsPedSwimming(playerPed) then
							exports['mythic_notify']:DoHudText('inform', 'You are Feeling Thirsty')
							SetPedToRagdoll(playerPed, 5000, 5000, 0, false, false, false)
							Wait(5000)
						end
			...
			if prevHealth >= 1 then
				...
				if stressVal >= 950000 then
					local chance = math.random(100)
					if chance < 30 then
						exports['mythic_notify']:DoHudText('inform', 'You Suddenly Blacked Out')
				...
				elseif stressVal >= 650000 then
					Citizen.Wait(5000)
					ShakeGameplayCam('LARGE_EXPLOSION_SHAKE', 0.07)
				elseif stressVal >= 550000 then
					Citizen.Wait(6000)
