...
function createDisplayItem(item, item, slot, price, type)
    local max = 100
    return {
        id = item.name,
        itemId = item.name,
        ...
        label = item.label,
        type = type or 'item',
        max = getItemDataProperty(item.name, 'max') or max,
        stackable = true,
        unique = item.rare,
        usable = item.usable,
        giveable = true,
        description = getItemDataProperty(item.name, 'description'),
        weight = getItemDataProperty(item.name, 'weight'),
        metadata = {},
        staticMeta = {},
        canRemove = item.canRemove,
        price = price or 0,
        needs = false,
        closeUi = getItemDataProperty(item.name, 'closeonuse'),
    }
end
    ...
    InvType[type].getInventory(identifier, function(inventory)
        ...
        for k, v in pairs(inventory) do
            if k ~= 'cash' and k ~= 'black_money' then
                ...
                local item = createDisplayItem(v, Item, tonumber(k))
                item.usable = false
                item.giveable = false
