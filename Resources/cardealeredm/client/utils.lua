...
RegisterNetEvent('carealeredm:ShowNotifyESX')
AddEventHandler('carealeredm:ShowNotifyESX', function(msg)
	ShowNotifyESX(msg)
end)
...
function ProduceNumberPlate()
	...
	while true do
		...
		ESX.TriggerServerCallback('carealeredm:PlateInUse', function (PlateInUse)
	...
	if option == "repay" then
		if plate ~= nil or not plate == '' then
			if amount ~= nil or amount >= 1 then
				ESX.TriggerServerCallback('carealeredm:GetOwnedVehByPlate', function(vehPlate, vehPrice, vehHash, vehFinance, vehRepaytime)
...
function DrawTextOptions(carPos,currentName,currentDownpayment,currentCommission,swapCar,currentPrice,currentStock)
	if (PlayerData.job ~= nil and PlayerData.job.name == Config.CarDealerJobLabel) then
		...
		-- finance:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.48, "~g~[U]~s~ Finance | ~r~"..currentDownpayment.."%~s~ Downpayment | ~o~$"..comma_value(math.floor((currentPrice + (currentPrice * (currentCommission / 100))) * (currentDownpayment/100))).."~s~")
		-- cost display:
		...
		-- buttons:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.54, " ~g~[E]~s~ Buy | ~y~[G]~s~ Swap Vehicle | ~p~"..currentCommission.."%~s~ Commission~s~")
		-- finance:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.48, "~g~[U]~s~ Finance | ~r~"..currentDownpayment.."%~s~ Downpayment | ~o~$"..comma_value(math.floor((currentPrice + (currentPrice * (currentCommission / 100))) * (currentDownpayment/100))).."~s~")
		-- cost display:
		DrawText3Ds(carPos[1],carPos[2],carPos[3] + 0.42, "~y~"..Config.InterestRate.."%~s~ Interest Rate | ~o~$"..comma_value(math.floor((currentPrice + (currentPrice * (currentCommission / 100))) * ((Config.InterestRate/100)+1))).."~s~ Total Cost ")
