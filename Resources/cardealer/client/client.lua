							...
							if (carDist2 <= Config.DrawTxtDist and carModel2 == carModel) then
								...
								if not IsPedInAnyVehicle(player, true) and not BuyInProgress then
									...
									if (IsControlJustPressed(0, Config.KeyToConfirmBuyVeh) or IsDisabledControlJustPressed(0, Config.KeyToConfirmBuyVeh)) then
										ESX.TriggerServerCallback('cardealer:GetPlayerMoney', function(hasMoney, carPrice, carCommission, price, payment)
											BuyInProgress = true
											if hasMoney then
												local VehCoords = nil
												local Heading = 0
							...
							if (carDist2 <= Config.DrawTxtDist and carModel2 == carModel) then
								...
								if not IsPedInAnyVehicle(player, true) and not FinanceInProgress then
									...
									if (IsControlJustPressed(0, Config.KeyToFinanceVeh) or IsDisabledControlJustPressed(0, Config.KeyToFinanceVeh)) then
										ESX.TriggerServerCallback('cardealer:GetFinancingMoney', function(hasUpfront, downPayment, carPrice, commission, payment)
...
function OpenShopMainMenu()
	...
	ESX.TriggerServerCallback('cardealer:GetDealerCount', function(DealerOnline)
...
function OpenVehicleSaleMenu()
	ESX.TriggerServerCallback('cardealer:GetAllOwnedVehicles', function(results)
		...
		function(data, menu)
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_veh_sale_confirm",
				...
				if data.current.value == "yes" then
					...
					ESX.TriggerServerCallback('cardealer:SellOwnedVehicle', function(sold, sellPrice, plate, model)
						if sold then
...
function PayFinancialBill(plate,price,model,finance,repaytime)	
	...
	function(data, menu)
		...
		if amount < difference then
			...
			ESX.TriggerServerCallback('cardealer:RepayAmount', function(hasPaid) 
				if hasPaid then 
...
function BuyShopMenuCar(buyVeh, vehGroup, category)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), "cardealer_shop_confirm",
		...
		if data.current.value == "yes" then
			...
			ESX.TriggerServerCallback('cardealer:ShopGetPlyMoney', function(gotMoney, price, payment)
				if gotMoney then
					local VehCoords = nil
					local Heading = 0
					for r,t in pairs(Config.PurchasedVehSpawn) do
...
function OpenEmployeesMenu(jobNameDB)
	...
	function(data, menu)
		...
		if data.current.value == "open_employee_list" then
			ESX.TriggerServerCallback('cardealer:getEmployees', function(employees)
				local elements = {
				...
				ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'cardealer_employee_list_' .. jobNameDB, elements, function(data2, menu2)
					...
					if data2.value == 'promote' then
						menu2.close()
						ESX.TriggerServerCallback('cardealer:getJob', function(job)
							local elements = {}
							...
							ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cardealer_promote_employee_' .. jobNameDB, {
								...
								ESX.TriggerServerCallback('cardealer:setJob', function()
									OpenEmployeesMenu(jobNameDB)
		...
		elseif data.current.value == "open_reqruit_menu" then
			ESX.TriggerServerCallback('cardealer:getOnlinePlayers', function(players)
				local elements = {}
...
function OpenAccountsMenu()
	...
	ESX.TriggerServerCallback('cardealer:GetAccountMoney', function(accountMoney,moneyOnPlayer)
...
function OpenAccountsDialog(option, label, accountMoney, moneyOnPlayer)
	...
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'cardealer_accounts_dialog', {
		...
		if amount == nil or amount == '' then
			...
			ESX.TriggerServerCallback('cardealer:Accounts'..callback..'', function(approved)
				if approved then
					menu.close()
...
Citizen.CreateThread(function()
    ...
    AddEventHandler('cardealer:UpdateCarDealerStockCL', function(vehModel,vehPlate)
        ...
        ESX.TriggerServerCallback('cardealer:UpdateDealerStock', function(stockUpdated)
    ...
    function GetCarDealerData()
        ESX.TriggerServerCallback("cardealer:FetchData", function(data1, data2, data3)
            vehicles = data1
            display = data2
            categories = data3
        end)
