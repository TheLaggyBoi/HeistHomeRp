...
ESX.RegisterServerCallback('cardealer:GetDealerCount', function(source, cb, option)
            ...
            if result[1] ~= nil then
				...
				if myID == CarOwner then
					...
					data = {}
						...
						TriggerEvent("esx:transfervehicle", p, myName, targetSteamName)
				else
...
AddEventHandler('cardealer:CheckFinanceStatus', function()
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier",{['@identifier'] = xPlayer.identifier}, function(results) 
		...
		if foundOwedVeh then
			...
			TriggerClientEvent("cardealer:ShowNotifyESX",xPlayer.source,_U('veh_repossessed_warning',warnTime))
...
-- ACCOUNT MENU:
ESX.RegisterServerCallback('cardealer:GetAccountMoney', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('cardealer:AccountsWithdraw', function(source, cb, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local account = MySQL.Sync.fetchAll('SELECT * FROM addon_account_data WHERE account_name=@account_name', {['@account_name'] = 'society_cardealer'})	
...
ESX.RegisterServerCallback('cardealer:AccountsDeposit', function(source, cb, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
	local approved = false
	if Config.UseCashMoney then
...
-- EMPLOYEE MENU:
ESX.RegisterServerCallback('cardealer:getEmployees', function(source, cb, society)		
	-- Fetch employees:
...
-- Set Job:
ESX.RegisterServerCallback('cardealer:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.job.grade_name == 'boss' then
		...
		if xTarget then
			...
			if type == 'hire' then
				TriggerClientEvent('cardealer:ShowNotifyESX', xTarget.source, _U('you_have_been_hired', job))
			elseif type == 'promote' then
				TriggerClientEvent('cardealer:ShowNotifyESX', xTarget.source, _U('you_have_been_promoted'))
			elseif type == 'fire' then
				TriggerClientEvent('cardealer:ShowNotifyESX', xTarget.source, _U('you_have_been_fired', xTarget.getJob().label))
			end
			cb()
		else
...
ESX.RegisterServerCallback('cardealer:getOnlinePlayers', function(source, cb)
...
ESX.RegisterServerCallback('cardealer:getJob', function(source, cb, society)
...
ESX.RegisterServerCallback('cardealer:GetFinancingMoney', function(source, cb, dPayment, model, stock)	
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('cardealer:SellOwnedVehicle', function(source, cb, plate, price, model)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('cardealer:ShopGetPlyMoney', function(source, cb, model, price)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('cardealer:GetPlayerMoney', function(source, cb, model, price, stock)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('cardealer:GetOwnedVehByPlate',function(source, cb, plate)	
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE owner=@identifier",{['@identifier'] = xPlayer.identifier}, function(vehData) 
		...
		if not vehFound then
			TriggerClientEvent('cardealer:ShowNotifyESX', xPlayer.source, _U('not_own_that_plate'))
		end
...
ESX.RegisterServerCallback('cardealer:RepayAmount', function(source, cb, plate, amount)
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('cardealer:GiveRegistrationPaper', function(player, target, plate)
	...
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner=@identifier',{['@identifier'] = xPlayer.identifier}, function(vehData) 
		...
		if vehFound then
			...
			TriggerClientEvent('cardealer:ShowNotifyESX', xPlayer.source, _U('veh_owner_change'))
			TriggerClientEvent('cardealer:ShowNotifyESX', tPlayer.source, _U('veh_owner_change2'))
		else
			TriggerClientEvent('cardealer:ShowNotifyESX', xPlayer.source, _U('plate_not_exists'))
		end
	end)
end)
...
ESX.RegisterServerCallback("cardealer:FetchData", function(source, cb)
	vehicles = {}
	display = {}
	categories = {}
	-- Fetch Data:
...
ESX.RegisterServerCallback('cardealer:GetAllOwnedVehicles', function(source, cb, option)
    local xPlayer = ESX.GetPlayerFromId(source)
    local plyVehicles = {}
...
ESX.RegisterServerCallback('cardealer:UpdateDealerStock', function(source, cb, VehModel)
    local xPlayer = ESX.GetPlayerFromId(source)
    -- Get Stock_
...
ESX.RegisterServerCallback('cardealer:PlateInUse', function (source, cb, plate)
