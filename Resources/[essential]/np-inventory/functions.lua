...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon)
    print('run use here?',itemid, slot, inventoryName, isWeapon)
    if itemid == nil then
        return
    end
    ...
    if (isWeapon) then
        ...
        if curWeapon == GetHashKey('WEAPON_UNARMED') then
            TriggerEvent("hud-display-item",itemid,"Unholsterd")
            print(GetCurrentPedWeapon(PlayerPedId(), true))
        else
            TriggerEvent("hud-display-item",itemid,"Holster")
            print(GetCurrentPedWeapon(PlayerPedId(), true))
        end
       --local itemInfo = GetItemInfo(slot)
       --print(json.encode(itemInfo))
       --if tonumber(itemInfo.quality) > 0 then
            TriggerEvent("equipWeaponID",itemid, "", itemid)
        --end
        print(GetCurrentPedWeapon(PlayerPedId(), true))
    ...
    if (itemid == "ticket") then
        TriggerEvent("scratchoffs:view")
        remove = true
      end
    ...
    if (itemid == "mobilephone") then
        TriggerEvent('xenknight:forceOpenPhone')
    end
    ...
    if (itemid == "scanner") then
        TriggerEvent('sup_radiobreaker')
        remove = false
    end
    ...
    if (itemid == "sandwich" or itemid == "hamburger") then
        print('this is hamburger')
        TriggerServerEvent('orp_inventory:Buger')
    ...
    if (itemid == "cola" or itemid == "water") then
        ...
        TriggerEvent('esx_status:add','thirst', 400000)
    end
    ...
    if (itemid == "methbag") then
        local finished = exports["reload-skillbar"]:taskBar(2500,10)
        if (finished == 100) then
            TriggerEvent("attachItemObjectnoanim","crackpipe01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["reload-skillbar"]:taskBar(25000,3)
            ...
            local finished = exports["reload-skillbar"]:taskBar(2200,10)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["reload-skillbar"]:taskBar(math.random(5000,25000),math.random(10,20))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["reload-skillbar"]:taskBar(15000,math.random(10,20))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["reload-skillbar"]:taskBar(timer,math.random(5,15))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
...
function getCash()
    return ESX.GetPlayerData().money ~= nil and ESX.GetPlayerData().money or 0
end
