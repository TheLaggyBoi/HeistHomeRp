...
ESX.RegisterServerCallback("t1ger_keys:fetchInsuranceData", function(source, cb)
	...
	local insuredCars = {}
	if xPlayer then
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner=@identifier',{['@identifier'] = xPlayer.identifier}, function(data) 
...
AddEventHandler("t1ger_keys:lendCarKeys", function(target, plate)
	...
	carKeys[carPlate] = {}
	-- insert into table:
	table.insert(carKeys[carPlate], {identifier = tPlayer.identifier})
	-- Sync data:
	TriggerClientEvent('t1ger_keys:syncTableKeys', target, carKeys, tPlayer.identifier)
...
AddEventHandler("t1ger_keys:stolenCarKeys", function(plate)
	...
	carKeys[carPlate] = {}
	-- insert into table:
	table.insert(carKeys[carPlate], {identifier = xPlayer.identifier})
	-- Sync data:
	TriggerClientEvent('t1ger_keys:syncTableKeys', source, carKeys, xPlayer.identifier)
end)
