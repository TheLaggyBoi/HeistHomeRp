...
AddEventHandler("mdt:getOffensesAndOfficer", function()
	...
	MySQL.Async.fetchAll('SELECT * FROM fine_types', {
		...
		local officer = GetPlayerName(usource)--GetCharacterName(usource)
...
AddEventHandler("mdt:getOffenderDetailsById", function(char_id)
	...
	MySQL.Async.fetchAll('SELECT * FROM `users` WHERE `id` = @id', {
		...
		GetLicense(offender.identifier, function(license) offender.license = license end)
		while offender.license == nil do Citizen.Wait(0) end
		MySQL.Async.fetchAll('SELECT * FROM `user_mdt` WHERE `char_id` = @id', {
			['@id'] = offender.id
		}, function(result)
			offender.notes = ""
	...
	for _, src in pairs(players) do
		if src ~= 0 and GetPlayerName(src) then
			...
			MySQL.Async.fetchAll('SELECT * FROM `users` WHERE `identifier` = @identifier', {
				['@identifier'] = xPlayer.identifier --GetPlayerIdentifiers(src)[1]
			}, function(result)
