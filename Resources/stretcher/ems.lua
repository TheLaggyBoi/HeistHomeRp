...
strNames = { 'v_med_bed1', 'v_med_bed2','prop_ld_binbag_01','v_med_emptybed'} -- Add more model strings here if you'd like
strHashes = {}
...
function StreachertoCar()
    ...
    if DoesEntityExist(closestObject) then
        if GetVehiclePedIsIn(ped, false) == 0 and DoesEntityExist(veh) and IsEntityAVehicle(veh) then
            AttachEntityToEntity(closestObject, veh, 0.0, 0.0, -3.7, 0.0, 0.0, 0.0, 0.0, false, false, true, false, 2, true)
...
RegisterCommand('spawnstr', function() 
    ...
    local str = CreateObject(GetHashKey('v_med_emptybed'), GetEntityCoords(PlayerPedId()), true)
    else
...
RegisterCommand('delStr', function(source, args)
	local object = GetHashKey('v_med_emptybed')
...
Citizen.CreateThreadNow(function()
	while true do 
        ...
        local pedCoords = GetEntityCoords(ped)
        local closestObject = GetClosestObjectOfType(pedCoords, 10.0, GetHashKey("v_med_emptybed"), false)
        if DoesEntityExist(closestObject) then
...
AddEventHandler('sit', function(strObject)
  ...
  if IsPedDeadOrDying(playPed) then
    revivePed(playPed)
    AttachEntityToEntity(PlayerPedId(), strObject, 0, 0, 0.0, 2.1, 0.0, 0.0, 0.0, 0.0, false, false, false, false, 2, true)
